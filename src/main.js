...
const setData = (socialObjet) => {
  ...
  totalFollowers.textContent = `Total Followers:  ${new Intl.NumberFormat("es-AR").format(followers)}`

  overviews.forEach(overview =>{
    overview.children[1].textContent = formatNumber(getRandomInt(0, 999999))
    overview.children[2].textContent = `${formatNumber(getRandomInt(0, 19999))}%`
    if  (getRandomInt(0, 2) === 1)
